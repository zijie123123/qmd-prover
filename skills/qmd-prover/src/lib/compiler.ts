...
import { AUX, atomicJson, atomicWrite, cleanId, exists, readJson, relativePosix, sha256, stableJson } from './files.js';
...
interface DiscoveredFile { absolute: string; relative: string }
...
async function discoveryExclusions(root: string, config: QmdProverConfig): Promise<Exclusion[]> {
...
async function discover(directory: string, root: string, exclusions: Exclusion[], output: DiscoveredFile[] = []): Promise<DiscoveredFile[]> {
