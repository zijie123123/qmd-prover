...
function traverse(graph: DependencyGraph, start: string, reverse = false): Set<string> {
...
function boundedInteger(value: unknown, fallback: number, { name, min, max }: { name: string; min: number; max: number }): number {
...
function shortestPath(graph: DependencyGraph, start: string, goal: string, reverse = false): string[] | null {
...
function subgraph(graph: DependencyGraph, ids: Iterable<string>): DependencyGraph {
