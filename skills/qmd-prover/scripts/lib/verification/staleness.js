...
export async function checkStaleness(root = process.cwd(), options = {}) {
    ...
    return withWriteLock(root, async () => {
        ...
        if (!compilation.complete)
            throw new Error('Cannot check staleness while QMD parsing fails');
        ...
        for (const [id, entry] of Object.entries(index).sort(([left], [right]) => left.localeCompare(right))) {
            ...
            const reasons = [];
            if (!result)
                reasons.push('fact-missing');
            else {
                if (result.statement_hash !== entry.statement_hash)
                    reasons.push('statement-changed');
                if (result.title_hash !== entry.title_hash)
                    reasons.push('title-changed');
                if (result.kind !== entry.kind)
                    reasons.push('kind-changed');
                if (result.proof_hash !== entry.proof_hash)
                    reasons.push('proof-changed');
                if (result.marker !== 'VERIFIED')
                    reasons.push('verified-marker-missing');
                ...
                if (stableJson(previousIds, 0) !== stableJson(currentIds, 0))
                    reasons.push('dependency-edge-set-changed');
                else {
                    for (const dependency of currentIds) {
                        if (currentDependencySnapshot[dependency] !== previousDependencies[dependency])
                            dependencyRoots.set(dependency, {
            ...
            if (entry.external_basis_hash !== currentExternalBasisHash)
                reasons.push('external-basis-changed');
            if (reasons.length) {
        ...
        for (const result of compilation.manifest.results) {
            if (result.marker !== 'VERIFIED' || result.status === 'verified' || index[result.id]?.status === 'verified' || changedIds.has(result.id))
                continue;
            changed.push({
                id: result.id,
        ...
        try {
            ...
            for (const [file, source] of sourceFiles)
                await atomicWrite(file, source.next);
            for (const [file, evidence] of evidenceFiles)
                await atomicJson(file, evidence.next);
            await atomicJson(indexFile, index);
            const rebuilt = await compileProject(root, options);
            if (!rebuilt.complete)
                throw new Error('Post-invalidation inspection could not rebuild a complete graph');
            ...
            for (const [file, source] of sourceFiles)
                await atomicWrite(file, source.original);
            for (const [file, evidence] of evidenceFiles)
                await atomicWrite(file, evidence.original);
