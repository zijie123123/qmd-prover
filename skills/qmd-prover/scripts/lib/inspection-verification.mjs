...
import { readLocatedBlock, setFactMarker } from './source.mjs';
...
import {
  ...
  checkerContract,
  invokeVerifier,
  readVerifierDecision,
  verifierErrorDetails,
  verifierDecisionLocation,
  verificationKey
} from './verifier.mjs';

const reservedMarkers = new Set(['OPEN', 'REJECTED', 'VERIFIED', 'REVOKED']);
...
function cleanSemanticText(text, kind, markerAtEnd = false) {
  ...
  if (kind === 'definition' || markerAtEnd) {
    for (let index = lines.length - 1; index >= 0; index -= 1) {
      ...
      if (reservedMarkers.has(lines[index].trim())) lines.splice(index, 1);
      break;
    }
  } else {
...
export function programmaticEligibility(compilation, result) {
  ...
  if (result.marker === 'REVOKED' || result.status === 'revoked') return { ready: false, reason: 'revoked' };
  if (result.kind !== 'definition' && !result.proof_present) return { ready: false, reason: 'proof-missing' };
  const failedReferences = (result.reference_checks ?? []).filter((check) => (
    check.existence !== 'pass'
    || check.scope !== 'pass'
    || check.status !== 'pass'
    || check.cycle !== 'pass'
  ));
  if (failedReferences.length) return { ready: false, reason: 'reference-check-failed', references: failedReferences };
...
async function applyDecision(root, id, expectedKey, packet, recordLocation, record, options) {
  return withWriteLock(root, async () => {
    ...
    if (!result) throw Object.assign(new Error(`@${id} disappeared while verification was running`), { code: 'INSPECTION_SOURCE_STALE' });
    const eligibility = programmaticEligibility(current, result);
    if (!eligibility.ready) throw Object.assign(new Error(`@${id} changed and is no longer eligible for verification`), { code: 'INSPECTION_SOURCE_STALE' });
    ...
    try {
      ...
      if (!rebuilt.complete || rebuiltResult?.status !== 'verified') throw new Error('Post-write inspection did not confirm a verified fact');
    } catch (error) {
...
export async function verifyCanonicalFact(root, requested, options = {}) {
  ...
  if (!result) return { status: 'error', code: 'INSPECTION_SOURCE_STALE', error: `Unknown fact: @${id}` };
  const existing = await storedAiCheck(root, result);
  if (existing) return existing;
  const eligibility = programmaticEligibility(compilation, result);
  if (!eligibility.ready) return { status: 'not-run', reason: eligibility.reason, programmatic: eligibility };

  let packet;
  try { packet = await packetForResult(root, compilation, result); }
  catch (error) { return { status: 'error', code: error.code ?? 'INSPECTION_SOURCE_STALE', error: error.message }; }
  ...
  if (cached.record) {
    ...
    let report;
    try { report = await invokeVerifier(packet, compilation.config); }
    catch (error) {
      const code = error.code ?? 'VERIFIER_FAILED';
      ...
      await atomicJson(path.join(root, AUX, 'reports', `inspection-${key.replace(/^sha256:/, '').slice(0, 24)}-failed.json`), failure);
      return {
  ...
  try { return await applyDecision(root, id, key, packet, location, record, options); }
  catch (error) {
    return {
      status: 'error', code: error.code ?? 'INSPECTION_WRITE_FAILED', error: error.message,
...
function skippedAiCheck(result, fatal = null) {
  ...
  if (result.status === 'revoked') return { status: 'not-run', reason: 'Verification was explicitly revoked.' };
  if (result.status === 'stale') return { status: 'not-run', reason: 'Stale verification could not yet be repeated.' };
...
export async function inspectCanonicalScope(root, select, options = {}) {
  ...
  try { staleness = await checkStaleness(root, options); }
    ...
    stalenessFailure = { code: 'STALENESS_CHECK_FAILED', message: error.message };
  ...
  if (!fatal && compilation.complete) {
    ...
    for (let transition = 0; transition < maximumTransitions; transition += 1) {
      ...
      for (const result of select(compilation).slice().sort((left, right) => left.id.localeCompare(right.id))) {
        if (!retryableStatuses.has(result.status)) continue;
        if (!programmaticEligibility(compilation, result).ready) continue;
        const outcome = await verifyCanonicalFact(root, result.id, options);
        aiChecks.set(result.id, outcome);
        if (outcome.source === 'independent-verifier' || outcome.attempted) verifierCalls += 1;
        if (outcome.cached) cacheHits += 1;
        if (outcome.status === 'error' && outcome.fatal) { fatal = outcome; break; }
  ...
  for (const result of selected) {
    ...
    if (check?.status === 'fail') diagnostics.push({
    ...
    if (check?.status === 'error') diagnostics.push({
