...
export async function submitProof(root, proposalFile, options = {}) {
    ...
    if (!initial.ok)
        throw new Error('Project has structural errors; repair them before submitting a proof');
    ...
    if (proposalErrors.length)
        throw new Error(`Proposal is structurally invalid: ${proposalErrors.map((item) => item.message).join('; ')}`);
    ...
    if (proposalProof.marker)
        throw new Error('A proposal must not contain OPEN, REJECTED, VERIFIED, or REVOKED control markers');
    ...
    if (canonicalTarget && proposedResult)
        throw new Error(`Proposal must not redefine existing canonical result @${proposalProof.target}`);
    if (proposedResult && proposedResult.id !== proposalProof.target)
        throw new Error(`Proposal proof must target its proposed result @${proposedResult.id}`);
    if (!canonicalTarget && !proposedResult)
    ...
    if (!candidateResult.proof_present)
    ...
    for (const dependency of candidateResult.dependencies) {
        ...
        if (!result)
            throw new Error(`Proposal dependency @${dependency} does not exist`);
        if (result.file !== destinationRelative) {
            ...
            if (!imported)
                throw new Error(`Proposal dependency @${dependency} is not imported by ${destinationRelative}`);
        }
        if (result.status !== 'verified')
            throw new Error(`Proposal dependency @${dependency} is not verified`);
    }
    ...
    if (!cachedDecision.record)
        writes.push(atomicJson(cachedDecision.location.file, reportRecord));
    ...
    return withWriteLock(root, async () => {
        const current = await compileProject(root, { ...options, excludeFiles: [proposalFile] });
        if (!current.ok)
            throw new Error('Project became structurally invalid while verification was running');
        const currentTarget = current.manifest.results.find((result) => result.id === target.id);
        const targetIsCurrent = isNewResult
            ? !currentTarget
            : currentTarget && currentTarget.statement_hash === target.statement_hash && currentTarget.proof_hash === target.proof_hash;
        ...
        let original = null;
        try {
            original = await readFile(destination, 'utf8');
        }
        catch (error) {
        ...
        try {
            ...
            if (!rebuilt.ok || mergedTarget?.status !== 'verified')
                throw new Error('Post-merge inspection did not confirm a verified target');
        }
        catch (error) {
            if (original == null)
...
export async function revokeVerification(root, requested, reason, options = {}) {
    if (!reason?.trim())
        throw new Error('Revocation requires a nonempty --reason');
    ...
    return withWriteLock(path.resolve(root), async () => {
        ...
        if (result.status !== 'verified')
            throw new Error(`@${id} is not currently verified`);
        const indexFile = path.join(root, AUX, 'verification', 'index.json');
        const index = await readJson(indexFile, {});
        ...
        try {
            ...
            if (!rebuilt.ok || rebuilt.manifest.results.find((item) => item.id === id)?.status !== 'revoked')
                throw new Error('Post-write inspection did not confirm revocation');
        }
        catch (error) {
