...
import { externalPolicyHash } from './external.mjs';
import { AUX, readJson, sha256, stableJson } from './files.mjs';
...
export function verifierErrorDetails(error) {
  return {
    ...
    ...(error?.exit_code !== undefined ? { exit_code: error.exit_code } : {}),
    ...(error?.signal ? { signal: error.signal } : {}),
    ...(error?.stderr ? { stderr_excerpt: excerpt(error.stderr) } : {}),
    ...(error?.stdout ? { stdout_excerpt: excerpt(error.stdout) } : {})
  };
}
...
export async function invokeVerifier(packet, config = {}) {
  ...
  try {
    ...
    throw new VerifierError('exit', `Verifier process could not run: ${error?.message ?? String(error)}`, {
      command: executable.command,
      cause: error
    });
  }
