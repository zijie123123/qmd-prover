...
function excluded(relative, exclusions) {
    ...
    for (const entry of exclusions) {
        ...
        if (new RegExp(`${prefix}${escaped}(?:/|$)`).test(relative))
...
export async function discoveryExclusions(root, config) {
    let ignored = [];
    try {
...
export async function discover(directory, root, exclusions, output = []) {
    for (const entry of (await readdir(directory, { withFileTypes: true })).sort((a, b) => a.name.localeCompare(b.name))) {
        const absolute = path.join(directory, entry.name);
        const relative = relativePosix(root, absolute);
        if (excluded(relative, exclusions) && !(entry.isDirectory() && mayReincludeDescendant(relative, exclusions)))
            continue;
        if (entry.isDirectory())
            await discover(absolute, root, exclusions, output);
        else if (entry.isFile() && entry.name.endsWith('.qmd'))
            output.push({ absolute, relative: relativePosix(root, absolute) });
    }
    return output;
}
