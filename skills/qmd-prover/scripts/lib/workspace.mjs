...
import { compileProject, findCycles, theoremBundle } from './compiler.mjs';
...
import { deriveGraphFindings } from './inspector.mjs';
import { readLocatedBlock, readLocatedProof } from './source.mjs';
import { checkStaleness } from './staleness.mjs';
import { accepted, buildVerifierPacket, checkerContract, invokeVerifier, verificationKey, verifierErrorDetails } from './verifier.mjs';
...
function cleanVerifierText(value, markerPosition = null) {
  ...
  if (markerPosition === 'first') {
    ...
    if (index >= 0 && reservedMarkers.has(lines[index].trim())) lines.splice(index, 1);
...
function workspaceStatus(result, marker = result.marker) {
...
function topologicalOrder(results) {
...
function canonicalContextFingerprint(compilation, target, available, externalBasis) {
...
function verifierFailure(error, target, inherited = false) {
  const details = inherited ? error.details : verifierErrorDetails(error);
  return {
    status: 'error',
    code: error.code ?? 'WORKSPACE_VERIFIER_FAILED',
    error: inherited
  ...
  for (const result of topologicalOrder(workspaceResults)) {
    ...
    if (cached.record) {
      ...
      try { await atomicJson(cached.location.file, record); }
        ...
        fatal = verifierFailure(Object.assign(new Error(`Verifier result for @${result.id} could not be cached safely: ${error.message}`), { code: 'WORKSPACE_CACHE_WRITE_FAILED' }), result.id);
        fatal.failed_target = result.id;
        outcomes.set(result.id, fatal);
        verification.stopped_after = result.id;
        continue;
